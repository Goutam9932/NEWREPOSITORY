...
public class EditDeleteGroupController {
    ...
    public String updateGroup(@PathVariable("id") Long id, @RequestParam("groupName") String groupName, Model model) {
        Group group = groupService.getGroupById(id);
        group.setGroupName(groupName);
        groupService.saveGroup(group);
        model.addAttribute("updateMessage", "Group updated successfully");
        model.addAttribute("group", group); // Add the updated group to the model
        return "editgroup"; // Replace with the actual Thymeleaf template name
    }
    ...
    // Other methods like loading the group page, etc.
}
