...
public class UserController {
	...
	public String registerUser(@ModelAttribute("user") User user, @RequestParam("imageFile") MultipartFile imageFile,
	...
	public String editUser(@ModelAttribute User user, @RequestParam("imageFile") MultipartFile imageFile, Model model) {
		try {
			...
			// Add the updated user to the model to reflect changes in the UI
			model.addAttribute("user", user);
		} catch (IOException e) {
			e.printStackTrace();
			model.addAttribute("error", "Error uploading image");
			return "user-edit"; // Return to the edit page in case of an error
		}
		return "redirect:/user/dashboard1"; // Redirect to the user dashboard after saving
	}

	@GetMapping("/userdetails")
	public String listUsers(@RequestParam(defaultValue = "0") int page, Model model, Principal principal) {
	    Pageable pageable = PageRequest.of(page, 4); // Adjust the size as needed
	    Page<User> usersPage = userService.findAllUsers(pageable);

	    // Fetch the logged-in user's details based on username
	    ...
	    model.addAttribute("loggedInAdminId", loggedInUser.getId());
